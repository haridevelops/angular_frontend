...
export class MovieService {
  ...
  constructor(private http: HttpClient) {
    ...
    this.springEndppoint = 'http://localhost:8080/api/movie';
  ...
  addMoviesToWatchlist(movie) {
    return this.http.post(this.springEndppoint, movie);
  }

  getMovieFromWatchlist(): Observable<Array<Movie>> {
    return this.http.get<Array<Movie>>(this.springEndppoint);
  }
  ...
  deleteFromWatchlist(movie: Movie) {
    return this.http.delete(this.springEndppoint + "/"+movie.id, {responseType: 'text'});
  }

  updateComments(movie) {
    const url = `${this.springEndppoint}/${movie.id}`;
